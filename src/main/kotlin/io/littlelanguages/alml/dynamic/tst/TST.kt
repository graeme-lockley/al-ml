...
data class AssignExpression<S, T>(val symbol: Binding<S, T>, val es: Expressions<S, T>) : Expression<S, T> {
    override fun typeOf(): Type? = symbol.typeOf()

    override fun yaml(): Any = singletonMap(
        "assign", mapOfType(
            typeOf(), Pair("symbol", symbol.yaml()), Pair("es", es.map { it.yaml() })
        )
    )
}

data class BinaryOpExpression<S, T>(
    val left: Expressions<S, T>, val op: BinaryOperator, val right: Expressions<S, T>, val lineNumber: Int
) : Expression<S, T> {
    override fun typeOf(): Type = op.typeOf()
...
fun BinaryOperator.typeOf(): Type = when (this.operator) {
    Operators.Plus, Operators.Minus, Operators.Multiply, Operators.Divide -> typeS32

    Operators.Equals, Operators.NotEquals, Operators.LessThan, Operators.LessEquals, Operators.GreaterThan, Operators.GreaterEquals -> typeBool
}

data class CallProcedureExpression<S, T>(val procedure: ProcedureBinding<S, T>, val es: List<Expressions<S, T>>, val lineNumber: Int) :
    Expression<S, T> {
    override fun typeOf(): Type? = procedure.typeOf()

    override fun yaml(): Any = singletonMap(
        "call-procedure", mapOfType(
            typeOf(), Pair("procedure", procedure.yaml()), Pair("es", es.map { e -> e.map { it.yaml() } }), Pair("line-number", lineNumber)
        )
    )
}

data class CallValueExpression<S, T>(val operand: Expressions<S, T>, val es: Expressions<S, T>, val lineNumber: Int) : Expression<S, T> {
    override fun typeOf(): Type? = operand.lastOrNull()?.typeOf()

    override fun yaml(): Any =
        singletonMap("call-value", mapOfType(typeOf(), Pair("operand", operand.map { it.yaml() }), Pair("es", es.map { it.yaml() })
        )
        )
}

data class IfExpression<S, T>(val e1: Expressions<S, T>, val e2: Expressions<S, T>, val e3: Expressions<S, T>?) : Expression<S, T> {
    override fun typeOf(): Type? = if (e3 == null) typeUnit
    else e2.lastOrNull()?.typeOf() ?: e3.lastOrNull()?.typeOf()

    override fun yaml(): Any {
        val result = mapOfType(typeOf(), Pair("e1", e1.map { it.yaml() }), Pair("e2", e2.map { it.yaml() })
        )

        return singletonMap(
            "if", if (e3 == null) result
            else result + Pair("e3", e3.map { it.yaml() })
        )
    }
}

data class SignalExpression<S, T>(val es: Expressions<S, T>, val lineNumber: Int) : Expression<S, T> {
    override fun typeOf(): Type = typeUnit

    override fun yaml(): Any = singletonMap("signal", es.map { it.yaml() })
}

data class IdentifierExpression<S, T>(val symbol: Binding<S, T>, val lineNumber: Int) : Expression<S, T> {
    override fun typeOf(): Type? = symbol.typeOf()

    override fun yaml(): Any = symbol.yaml()
}

data class TryExpression<S, T>(val body: IdentifierExpression<S, T>, val catch: IdentifierExpression<S, T>, val lineNumber: Int) : Expression<S, T> {
    override fun typeOf(): Type? = body.typeOf() ?: catch.typeOf()

    override fun yaml(): Any = singletonMap(
        "try", mapOfType(
            typeOf(), Pair("body", body.yaml()), Pair("catch", catch.yaml())
        )
    )
}

data class LiteralS32<S, T>(val value: Int) : Expression<S, T> {
    override fun typeOf(): Type = typeS32

    override fun yaml(): Any = value
}

data class LiteralString<S, T>(val value: String) : Expression<S, T> {
    override fun typeOf(): Type = typeString
